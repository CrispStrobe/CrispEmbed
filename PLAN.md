...
| other `ggml_conv_2d_direct` users (glm_ocr, got_ocr) → im2col? | `glm_ocr.cpp:806`, `got_ocr.cpp:842-852` | **Scoped candidate — verify dominance first (do NOT blind-swap)** | Sweep after the layout win: the only other `conv_2d_direct` sites are glm_ocr (1 vision patch-embed conv) and got_ocr (4 SAM-style neck convs). Unlike layout (whole backbone = conv_2d_direct, measured 99.6% GPU / 11.7s), these are **transformer-dominated** models where the convs are a small neck/patch-embed — likely marginal. The swap is cos≈1 + low-risk (same as layout), but per the "measure the dominant cost" rule, confirm the conv is a meaningful fraction (metal-prof the specific graph) before flipping. Both models are heavy → needs a quiet box (couldn't verify this session; box at loadavg 10–137). surya_det/ocr_detect already use `ggml_conv_2d` (not direct) — no action. |
