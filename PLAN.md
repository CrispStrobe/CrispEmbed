...
| # | Item | Owner |
|---|---|---|
| N1 | ~~PP-OCRv6 fused batch graph on Metal~~ **UNBLOCKED 2026-08-04** (`perf/ppocrv6-speed`, merged): the "Metal fourth-dimension pooling" abort was a zeroed in-out width parameter in the fused caller — every batch graph was built at width 0 and asserted on ANY backend. Width seeded: CPU two-crop contract byte-identical, Metal fused runs 4/4 crops scalar-identical at **54.9 vs 118.6 ms scalar (2.2x)**. Still opt-in (`CRISPEMBED_PPOCRV6_BATCH_GRAPH`, tiny-only). **Remaining:** extend the fused lane to the large-stem small/medium recognizers (the receipt-lane prize) + multi-fixture promotion sweep. `feat/ppocr-next-20260731` is fully merged — nothing unmerged to salvage | *(open, unowned)* |
