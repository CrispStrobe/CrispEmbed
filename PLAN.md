...
|-------|-------------------|------|--------|
| 2026-08-06 | `fix/glm-no-repeat-ngram` / `.claude/worktrees/fix-glm-ngram` | **Claimed: GLM-OCR repetition guard** — SubtitleEdit PR-13238 user feedback (Japanese PGS, CPU-only box): GLM-OCR q8 emits repeated words/chars; `glm_ocr.cpp` is the ONE VLM engine without the F1 `core_decode::argmax_no_repeat_ngram` guard (deepseek/got/internvl2/math/qwen2vl/unlimited all have it). Wire it with the qwen2vl pattern + value-parsed env; decoded-output gate on the glm fixtures | IN PROGRESS |
