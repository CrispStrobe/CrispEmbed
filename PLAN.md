...
|-------|-------------------|------|--------|
| 2026-08-07 | `fix/pix2struct-decoder` | **Claimed (new lane from the O3 A/B): pix2struct decoder correctness + profile.** (a) Output is raw comma-separated token ids — detokenizer missing?; (b) repetition degeneration on both doc fixtures; (c) ~80 ms/token for a 6L/256d decoder — profile for per-step rebuild / missing KV reuse before any GPU-graph talk (R5 lesson) | **IN PROGRESS** |
