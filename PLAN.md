...
|-------|-------------------|------|--------|
| 2026-08-06 | `probe/vulkan-bringup` (kernel `chr1s4/crispembed-vulkan-probe`) | **Claimed (O10): Vulkan bring-up probe** — one Kaggle kernel: vulkaninfo on the GPU image, a ggml Vulkan build, one tiny graph computed and verified. Deliverable is a yes/no + writeup; NO optimization claims | IN PROGRESS |
