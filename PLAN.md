...
| gate debug `fprintf` behind verbosity | layout_detect **DONE**; surya_det, ocr_detect open | **layout_detect DONE (verified)** | `layout_detect.cpp` `detect()` printed per-layer/per-call tensor-stat spew (`dec*_after_sa`, `dec0 values`, `dec0 cross_out …`, `dec%d_norm2/3`, `NOT FOUND`, `%zu detections`) to stderr with NO debug guard — and the backbone per-block block did ~20 `ggml_backend_tensor_get` GPU→CPU read-backs + min/max scans **every call** with only the *print* LDBG-gated. Gated all on `layout_debug()`. Verified on layout-heron-f32 + `scan_page_pd.png`: stderr spew 0 lines, 20 regions still detected identically (blocks are side-effect-free reads → output unchanged by construction). Bonus: skips the debug read-backs/scans in production. surya_det/ocr_detect deferred (need their models to runtime-verify their guards). |
| **layout_detect Phase-2 decoder = scalar `cpu_linear` matmul (MEASURED)** | `layout_detect.cpp:1018` | **Open — the real lever, project-sized** | `CRISPEMBED_LAYOUT_DETECT_BENCH` on layout-heron-f32 + `scan_page_pd.png`: **Phase 2 decoder ~25–48s dominates** (Phase 1 backbone ~11–13s, heads ~5ms). Correcting an earlier guess: the deformable *sampling* loop (`:1542`, ~17M MACs) is NOT the cost — the dominant cost is `cpu_linear` (`:1018`), a **fully scalar triple-loop matmul, un-SIMD, un-threaded, stride-N access**, called ~10×/layer × 6 with dims up to 256×256×8400 (~3–5G MACs). It also re-`tensor_to_f32`'s the weight per call, but that's marginal vs the matmul FLOPs (scunet lesson). Lever = SIMD-GEMM it (à la `linear_batch_cpu`) — but layout stores everything `[dim, N]` col-major vs `linear_batch_cpu`'s `[N, dim]` row-major, so it needs a layout adaptation, and the reduction-order change makes it cos≈1 not byte-identical (verify by region output, not bytes). **Deferred: needs a quiet box — this run had loadavg ~11–19 and 30–60s/run, where per handoff rules timing A/B "fabricates wins".** |
