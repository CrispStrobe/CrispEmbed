...
|-------|-------------------|------|--------|
| 2026-08-05 | `chore/repin-rerank-f7` (round-7 coordinator's own work) | **Reranker sub-Q8 re-pin decisions**: local Metal+CPU cross-check of the `-f7` artifacts (jina `-q4_k-imatrix`, bge-reranker-v2-m3 sub-Q8) vs the Kaggle x86 A/B (G3 precedent), then re-pin/alias decision + `model_hashes.h` + fresh-download SHA-verify. q8_0 stays default | **IN PROGRESS** |
| 2026-08-05 | *(delegated, round-7; coordinator verifies + merges)* | **mxbai DeBERTa q/k imatrix provenance A/B**: quantize.cpp direct-name-vs-merged-alias lookup for DeBERTa q/k (imatrix collected over rel-position inputs); A/B by decoded rerank scores vs official mixedbread-ai ONNX. Any quantize.cpp behavior change is a coordinator merge decision | **IN PROGRESS** |
| 2026-08-05 | *(delegated, round-7; coordinator verifies + merges)* | **UOCR_* output-affecting presence-gate sweep** (`src/unlimited_ocr.cpp`, ~40 gates — the DS_ mirror): per-gate decoded-output A/B, three spellings, ds-gates methodology, `core_env::on` | **IN PROGRESS** |
