...
|-------|-------------------|------|--------|
| 2026-07-31 | `feat/easyocr-ggml` / `.codex/worktrees/feat-easyocr-ggml` | **Picked:** unify CRAFT/DBNet/Tesseract-style segmentation with EasyOCR lines and LayoutLM/Tesseract words; then validate downstream OCR handoffs. Latest checkpoint: generic OpenCV CV_8U fixed-point resize structure is ported; identical-raster control proves graph/LSTM parity, while the official macOS ARM resize still leaves a one-level residual and the strict English width-128 logits gate remains open | **IN PROGRESS** |
...
| 2026-08-01 | `feat/ppocr-next-20260731` | **Picked:** O11.3 preprocessing/geometry cost split: `CRISPEMBED_PPOCRV6_DET_BENCH=1` now reports detector normalize, graph, and total timings; German CC0 CPU normalize is ~2.5 ms versus ~626 ms scalar detector work, so GPU effort stays focused on graph compute and geometry parity rather than moving trivial normalization | **IN PROGRESS** |
...
| 2026-08-01 | `feat/ppocr-next-20260731` | **Picked:** O11.7 PP-OCRv6 recognizer weight-cache optimization: cache immutable convolution dequantization across line crops; live tiny output remains valid, medium detector timing remains the dominant cost | **IN PROGRESS** |
