...
| 2026-07-13 | `feat/uocr-stacked-experts` | Unlimited-OCR: stacked MoE experts (same win as ds-ocr2 #4) | **Code landed on branch (`7d19c2e`), Kaggle reconvert RUNNING.** Same DeepSeek-V2 MoE (64 exp, 1280d) — converter+loader are a verbatim port of ds-ocr2 #4 (both compile). Kernel `chr1s4/crispembed-unlimited-ocr-stacked-convert` reconverts `baidu/Unlimited-OCR`, shard-aware byte-validates, quantizes q4_k, uploads f16+q4_k to `cstr/unlimited-ocr-crispembed-GGUF` as `-stacked` (non-clobbering). Next: local Metal A/B + land + registry promote + regression + docs. |
