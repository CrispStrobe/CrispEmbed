...
| 2026-07-16 | `feat/core-json-scalars` | Finish `core_json` centralization: route the server's ~14 hand-rolled scalar field parses (`conf`/`threshold`/`max_tokens`/`det_size`/`page_*`/`task` via `find`+`atof`/`atoi`) + `extract_image_path` through `json_extract_number` / `json_extract_strings`, all using the depth-1 `json_find_key_value` finder. Closes the B2 key-decoy bug for scalar params (was still live). | **IN FLIGHT.** New `json_extract_number` (structural/legacy/gated) reusing the B2 finder; unit A/B proves legacy is fooled by a decoy value while structural returns the default; `/scan/split` live decoy test (model-free). Bindings confirmed to NOT hand-roll JSON, so this closes the core_json story at server+CLI. |
| 2026-07-16 | `feat/json-arch-hardening` | **A1–A4 ecosystem-compat + input-parsing hardening** (backlog "Ecosystem-compat + input-parsing hardening"). A1 finish JSON parser migration (4 endpoints still carry the #34 bug, incl. `/embed`); A2 arch-driven hparams + strict mode (systemic #33); A3 community-GGUF import matrix; A4 red-main / toolchain-drift signal. | **ALL FOUR DONE — validated, pending push.** Each with env gate + A/B + unit tests. **A1** (`546b52b`): `/embed`,`/rerank`,`/ner`,`/kie` migrated — zero delimiter-scan parses remain; gate `CRISPEMBED_SERVER_LEGACY_JSON=1`; A/B legacy==escaped on all benign payloads; unit 23 + live 7/7 PASS. (The old scan was worse than thought: a `]` in the FIRST element drops *every* element, not one.) **A2** (`9315124`): hparams derived from `general.architecture` (`src/core/hparam_keys.h`); gates `CRISPEMBED_ARCH_HPARAMS=0` / `CRISPEMBED_STRICT_HPARAMS=1`; A/B MiniLM byte-identical on/off, nomic byte-identical to #33's alias build (so HF parity 0.9839 carries over), gate-off reproduces the pre-#33 GGML_ASSERT, strict turns it into a clean refusal, 0 false positives; unit 18 PASS. **A3** (`e824c91`): community-GGUF matrix + garbage guard; unit 15 PASS (no models), live nomic passes (margin 0.5619), and it correctly FAILS under `ARCH_HPARAMS=0` — the guard has teeth. **A4** (`d82049a`): `tools/check_workflow_pins.sh` (all 9 toolchain steps pinned) + daily `main-health.yml`; self-test 8 PASS both arms; retro-replayed at 2026-07-15 it flags 2/5 red — would have caught the outage a day early. Two bugs caught by driving the guards: awk `END` overriding `exit` status, and regex parens silently skipping the two watched workflows that were actually red. |
