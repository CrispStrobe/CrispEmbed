...
| 2026-07-13 | `feat/transcoda-omr` | Transcoda-59M zero-shot OMR (full-page score → Humdrum `**kern`; ConvNeXt-V2-tiny enc + 8-layer RoPE cross-attn decoder). **Clean-room** (weights CC-BY-4.0, code AGPL — engine written from paper + config + oracle only) | **PER-STAGE PARITY PASS** — engine `src/transcoda_ocr.{h,cpp}` + converter + oracle done; wired (CMake/dispatcher/CLI/registry). `test-transcoda-diff` **all stages cos_min=1.000000** (enc_grid/enc_raw/enc_pos + dec_block0..7 + logits, CPU & Metal). Native preproc bit-exact vs oracle (cos 1.0). KV-cache view-stale bug fixed. Greedy decode emits valid `**kern` matching oracle prefix. Next: confirm full==kv, quantize q8_0, upload `cstr/transcoda-omr-GGUF` (CC-BY card), regression fixture. |
