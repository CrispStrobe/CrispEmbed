...
|-------|-------------------|------|--------|
| 2026-08-06 | `fix/cuda-rec-zero-results` (branch pushed; kernel `chr1s4/crispembed-cuda-rec-fix` v1 RUNNING) | **Fable task 1: CUDA-rec 0-results — ROOT-CAUSED + FIXED, CUDA proof in flight.** Not CUDA-specific and not readback/CTC: `pp_graph_resident`'s native-quant path uploaded raw F32 bytes into a q8_0-typed resident (head weight) — misread f16 scale bytes ⇒ NaN logits ⇒ NaN-poisoned `max_element` returns index 0 = CTC blank ⇒ every crop decodes empty (`boxes=N results=0`, rc=0). **Reproduced on M1 Metal with the q8-head artifact** (Metal was only ever validated on f16); fix = same-type residents copy raw source bytes (host-staged). Gates: q8 strip 0→12 results conf 0.94, q8 fused graph == `NO_GRAPH` scalar reference byte-for-byte, f16 page byte-identical to baseline, 12/12 model-free CI. Kernel proves decoded text on CUDA (arms: q8-graph vs q8-scalar-ref vs f16 control); merge + O11 after it lands | IN PROGRESS |
