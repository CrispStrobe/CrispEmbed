...
| 2026-07-13 | `feat/texteller-preprocess` | **TexTeller was producing garbage — root-caused + fixed** (decoder FFN activation hardcoded ReLU, needs GELU; + preprocessing mean/std/pad). `src/math_ocr.cpp` now data-driven from GGUF; converter emits `decoder.activation_function` + `encoder.image_mean/std/preprocess_pad`. | **Code fix DONE + validated** (native decode byte-exact vs HF `OleehyO/TexTeller` on both formula fixtures, f16==q8_0, CPU==Metal; pix2tex byte-identical). **Remaining:** re-upload `cstr/texteller-3-GGUF` with the new metadata (metadata-patch, no re-quant) + add texteller regression fixture. Audit: `ppformulanet`/`_l` use tanh-GELU vs MBart erf (minor) and have `expected_text: null` fixtures. |
