...
| 2026-07-13 | `feat/flova-omr` | Flova/omr_transformer — handwritten/whiteboard OMR (donut-swin + mBART VED → LilyPond, Apache-2.0) | 🔄 **In progress** — evaluated (only permissive handwritten-music model; monophonic "simple notes"). Porting: reuses `mixtex_ocr` Swin + `math_ocr` VED decoder. Converter → dumper → engine → validate → quant/upload. |
...
| 2026-07-13 | opus-1m (interop/SR) | Kaggle reranker τ-eval on the expanded 30-group RERANK_EVAL corpus (`crispembed-imatrix-quant`) | **Batch 1 DONE** (bge-base/-v2-m3, jina-v2): imatrix cuts q4_k score-drift −23–29%, holds τ on bge, **improves** τ on jina (0.929→0.942); q4_k+imatrix = recommended sub-q8 reranker quant. **Batch 2 🔄** re-running the remaining 4 rerankers (ms-marco-MiniLM-L-6/-12, mxbai-rerank-base/-xsmall — currently on the stale n=5 corpus) to put the full 7-reranker roster on one n=30 corpus (FORCE). |
