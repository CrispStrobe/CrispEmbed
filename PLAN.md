...
| 2026-07-13 | `feat/math-ocr-fixtures` | pix2tex-mfr regression fixture | **Landed `main`** — `run_one --name pix2tex` cer 0.000 (formula_quadratic.png, `cstr/pix2tex-mfr-gguf`). |
| 2026-07-13 | `feat/texteller-preprocess` | **TexTeller was producing garbage — root-caused + fixed** (decoder FFN activation hardcoded ReLU, needs GELU; + preprocessing mean/std/pad). `src/math_ocr.cpp` now data-driven from GGUF; converter emits `decoder.activation_function` + `encoder.image_mean/std/preprocess_pad`. | **DONE + shipped.** Code fix on `main` (`e8d5a9f`); native decode byte-exact vs HF `OleehyO/TexTeller`, f16==q8_0, CPU==Metal, pix2tex byte-identical. `cstr/texteller-3-GGUF` re-uploaded (f16/q8_0/q4_k metadata-patched, no re-quant, all 3 decode correct; apache-2.0 card intact). Regression fixture `texteller` added (`run_one` cer 0.000 vs re-uploaded q8_0). **Follow-up (separate):** `ppformulanet`/`_l` use tanh-GELU where MBart is erf (minor cos ~0.999) and carry `expected_text: null` fixtures — validate end-to-end vs HF. |
...
| 2026-07-13 | `feat/smt-fp-fullpage` | SMT++ **full-page** pianoform OMR (`PRAIG/smt-fp-grandstaff`) — same `SMTModelForCausalLM` class as shipped `src/smt_ocr.cpp`, so config+weights+preprocessing job | **IN PROGRESS.** Config diff done: identical topology; deltas are maxh 256→2970, maxw 3056→2100, maxlen 1281→4353, vocab 20578→181. 2-D PE computed at runtime (no table overflow), pos1d/maxlen auto-scale from config → **only real code change = preprocessing** (FP_GrandStaff finetune = `reduce_ratio=1.0` + `RandomInvert(p=1.0)` invert + grayscale, vs base's clamp+no-invert). Plan: gate fp preproc behind new GGUF meta (`smt.preproc.reduce_ratio`/`.invert`); leave base path untouched. Next: run converter on fp checkpoint. |
...
| 1b | **SMT++ full-page** | ~10.9M | **MIT / MIT** — `PRAIG/smt-fp-grandstaff` (public, **not gated**, verified HF card) | **same `SMTModelForCausalLM` class as shipped SMT** (curriculum-trained for full page) | bekern | **Full-page pianoform** (no separate layout stage) | **Low** | **DOABLE — top permissive target. Handover: `handover-prompts/smt-plusplus-fullpage-handover.md`.** Verified: same network topology as our shipped `smt_ocr.cpp` (that engine was written from `modeling_smt.py`, the SMT++ class). The 2-1 "refutation" was a wording artifact (abstract credits "the architecture" = SMT's design, not a change). So it's a **config + weights + preprocessing** job, not a re-port. Three risk items to verify: input resolution, encoder 2-D PE table size (fixed learned → may need bigger/interp for full page), decoder `pos1d`/max-seq |
| 2 | **Transcoda-59M-zeroshot** | 58.8M | **AGPL code / CC BY 4.0 weights** (`btrkeks/transcoda-59M-zeroshot-v1`; cc-by-4.0 **triple-verified**: HF badge + rendered card + raw YAML `license:` + body text) | ConvNeXt-V2-Tiny enc (GRN) + 2-layer projector + 2D-sin PE → 8L pre-LN dec (d512/8h/ff1024, **RoPE self + cross-attn**) | **kern | **Full-page + historical scans** (zero-shot); **current OMR-NED SOTA** (Polish 63.97%, Verovio 18.46% — beats SMT++ & Legato) | **Med** | **DOABLE — accuracy leader. Handover: `handover-prompts/transcoda-engine-handover.md`** (full clean-room facts-spec). Weights CC BY 4.0 → GGUF redistribution clean (attribute). Engine **clean-room** (code AGPL — facts-spec + oracle, never transcribe). Arch in-tree: ConvNeXt-V2 ≈ SMT ConvNext + **GRN** (the one new delta), cross-attn decoder ≈ flova mBART, RoPE ≈ Qwen3, BPE ≈ core/bpe. Data `polish-scores` = `license: other` (irrelevant to CC-BY weights). Defer beam-3 + GBNF grammar decode |
...
| 5 | oemer | 2× U-Net | MIT / MIT | 2 segmentation U-Nets + **80–90% numpy reconstruction** + 3 SVMs | MusicXML | Printed, photos, skewed | High | Reference-only — **not an impossibility, a value call.** The U-Nets map trivially to ggml (same class as shipped DBNet/NAFNet); the blocker is that masks are worthless without the huge rule-based reconstruction (staffline extraction, note grouping, beam/flag distillation, MusicXML build) + 3 SVMs — a large brittle C++ reimpl with no neural payoff and no accuracy edge over the end-to-end transformers |
...
| ~~8~~ | ~~Clarity-OMR~~ | ~167M (668 MB f32) | **GPL-3.0 code / CC BY-SA 4.0 weights** (`clquwu/Clarity-OMR`, public, verified config.json) | pipeline: YOLOv8m staff-detect → **DaViT enc + 8L RoPE cross-attn dec** (`DaViT_RoPE_Decoder`, dim 768, 12 heads, DoRA rank 64, vocab 487, max 512) | notation tokens → MusicXML | printed **staff-level** (needs external staff detect) | Med–High | **Verified redundant** (not impossible). The stage-B recognizer IS an extractable VED, but: (a) **DaViT** encoder is a new backbone (spatial-window + *channel* attention — not in-tree), (b) **DoRA**-merge converter step, (c) **staff-level only** — duplicates shipped SMT/TrOMR with no capability gain, at ~8× SMT size, (d) stricter license (CC BY-**SA** ShareAlike + GPL code). Only worth the DaViT port if a *wanted* future model needs that backbone |
...
Scoped, self-standing handover briefs for the two doables live in the gitignored
`handover-prompts/` (`smt-plusplus-fullpage-handover.md`, `transcoda-engine-handover.md`).
