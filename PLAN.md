...
| 2026-08-01 | `feat/ppocr-next-20260731` | **Picked:** align PP-OCRv6 large-recognizer activation semantics with the recovered PaddleX source: StemBlock Conv-BN uses ReLU; LightSVTR conv branches use configured SiLU. Updated native CPU/graph and `dump_ppocrv6_reference.py`; small fox taps now pass through stage4 (`0.999958`), head input (`0.999992`), logits (`0.999996`), and full graph logits (`0.999995`). Both native and reference decode `上ai`, so text quality remains explicitly unvalidated | **COMPLETED — parity fixed, quality blocked** |
