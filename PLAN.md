...
|-------|-------------------|------|--------|
| 2026-08-05 | `feat/smoldocling-metal` / `.claude/worktrees/feat-smoldocling-metal` | **Claimed (G1=F4, round-4 coordinator's own work):** SmolDocling vision-backend split residency — vision+connector (SigLIP, 31.7 s of 37.3 s on fox) to Metal, 135M decode stays CPU; value-parsed FORCE_CPU-style gate. Gates: payload byte-identical CPU vs Metal on the 5-page T15 set, interleaved same-window timing, MTL0 per-run stderr, explicit `--gpu-backend` every arm. G2b queued after (serialized — one heavy consumer). G8 NOT claimed (CrispASR has an active session today) | **IN PROGRESS** |
