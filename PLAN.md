...
| 2026-07-31 | `feat/ppocr-next-20260731` | **Picked:** O11.2 PP-LCNet line/page orientation graph port: backend-scheduled depthwise/pointwise/SE blocks with CPU fallback and orientation parity gates; graph now uses the converter's canonical weight layout, correct `[1280,2]` linear view, and safely schedules Metal plus CPU for unsupported depthwise ops. Expanded Metal parity passes 9/10 German/Arabic/derived fixtures; uneven-illumination Arabic remains a numerical outlier (delta 1.07/3.22; CPU graph passes), so production still requires `PPLCNET_ORIENTATION_GRAPH_ACCEPT=1` | **IN PROGRESS** |
