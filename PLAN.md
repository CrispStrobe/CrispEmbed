...
|-------|-------------------|------|--------|
| 2026-08-06 | `perf/glm-grid-sizing` / `.claude/worktrees/perf-glm-grid` | **Claimed (GLM round 2): vision grid sizing for wide-short images** — measured: subtitle-shaped `scan_strip` costs −16% vs fox despite a fraction of the area ⇒ preprocessing pads thin strips toward a full grid. Find the resize/grid logic, check the model's native-aspect contract, fix the padding, A/B (vision ms + decoded text on strip/fox/CC0) | IN PROGRESS |
