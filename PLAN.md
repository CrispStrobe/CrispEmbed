...
8. **GLM-OCR vision Metal miscompute** [ARBITRATED 2026-08-07, fix OPEN]:
   the real HF forward cleared the reference (cos 1.000000 vs the published
   ref GGUF at all 24 layers), so the defect is the PORT — and only on
   Metal (CPU cos_glob 1.000000, Metal 0.958/0.940). This is a shipping
   quality bug for every Mac GLM-OCR user. Ruled out already: the
   reference, matmul precision, and per-layer structure. Remaining work is
   a Metal graph bisection on GENUINE truncated output (never `set_output`
   snapshots) — suspect the ops carrying the |x| ~1e5 outliers. Parity tool:
   `tools/hf_glm_vision_parity.py` (run it with `GLM_BLOCK_ORDER=1`).
