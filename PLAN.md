...
|-------|-------------------|------|--------|
| 2026-08-07 | *(kernel `chr1s4/crispembed-conv-ab` v3; script landed `d3d53d4a`)* | **Claimed (round N+3 task 6): conv-ab v3 — det-only dbnet CPU-vs-CUDA with box-level compare (`test-ocr-detect`, greedy center-match: count/coord-delta/score-delta), + the `LAYOUT_CONV_F16` arm for the T4 draw.** ⚠ First push landed on chr1str via the ROTATED default `access_token` (datasets silently dropped → would have cold-built; kernel deleted; `kaggle_usage.md` corrected — always export `KAGGLE_API_TOKEN` explicitly). v3 running under chr1s4 with ccache+token datasets attached | **IN PROGRESS** |
