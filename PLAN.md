...
| 2026-07-31 | `feat/ppocr-next-20260731` | **Picked:** O11.2 PP-LCNet line/page orientation graph port: backend-scheduled depthwise/pointwise/SE blocks with CPU fallback and orientation parity gates; graph now uses the converter's canonical weight layout and safely schedules Metal plus CPU for unsupported depthwise ops. Metal executes without abort, but diagnostic logits remain non-parity (1854/-1855 vs CPU 6.64/-22.77), so `PPLCNET_ORIENTATION_GRAPH_ACCEPT=1` is required and the shipped path remains CPU reference | **IN PROGRESS** |
