...
|-------|-------------------|------|--------|
| 2026-08-05 | `feat/ds2-dynamic-crop` / `.claude/worktrees/feat-ds2-dynamic-crop` | **Picked (round-3 G2, coordinator's OWN work):** DeepSeek-OCR2 dynamic-crop port per the F5 brief — reference crop logic read line-by-line at pinned revision `aaa02f38` (dynamic (0-6)×768² + 1×1024² global, crop_mode=True), gated separately from the F1 guard, then F1 matrix arms + gold gate re-run. Note: `~/.cache/hf-regression` is GONE despite the round-3 env notes (per-expert gold artifact will re-download) | **IN PROGRESS** |
| 2026-08-05 | `feat/metal-cache-cap-lanes` *(delegated; coordinator verifies before merge)* | **Picked (G4=F2):** Metal pipeline-cache cap adoption across the remaining `crispasr_init_gpu_backend()` lanes; per-lane init-time A/B (`CRISPEMBED_INIT_BENCH=1`) + decoded-output-unchanged on locally-cached models. The 683 MB archive deletion stays with the coordinator, after adoption is everywhere | **IN PROGRESS** |
| 2026-08-05 | `feat/kaggle-driver-failfast` *(delegated; coordinator verifies before merge)* | **Picked (G7d):** flip the three upload-bearing drivers without a vendored harness (`unlimited-ocr-convert`, `crispembed-splade-fix`, `deepseek-ocr2-convert`) to `resolve_hf_token(require=True)` before any upload work | **IN PROGRESS** |
