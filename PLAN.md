...
| 2026-08-01 | `feat/ppocr-next-20260731` | **Picked:** O11.1 full-graph implementation contract: persistent static-shape graphs, scheduler-selected backend, backend-resident/dequantized weights, reusable input/output staging, batched line crops, and CPU cosine/logit parity fallback; PP-OCRv6 tiny/small recognizer now runs one persistent graph through logits (backbone + pooled head + folded BN + projections) behind `CRISPEMBED_PPOCRV6_GRAPH=1`; production routing remains gated on tensor parity and Metal validation | **IN PROGRESS** |
