...
| 2026-08-04 | `feat/t14-deepseek2-decode-graph` (agent) | **T14 DONE — awaiting coordinator verify+merge.** `[deepseek-ocr2-stage-bench]` landed first (T12 gap). Persistent decode graph is now the DEFAULT: Metal interleaved 9-pair load-gated A/B, decode 11473.7->8191.5 ms (**1.40x**), total 15815.2->12784.8 ms (1.24x), decoded text byte-identical on all 25 gold fixtures (Metal) + CPU synth subset; prefill/sam/qwen2_enc unchanged. **Premise correction:** the per-layer rebuild this task targeted was only 1% of decode (26/5223 ms) — the win is 13 dispatches + 24 host<->device transfers per token collapsing to 1, so measure the overhead fraction before porting to qwen2vl/granite/smoldocling. **Trap:** copying qwen2vl's full-`max_seq` KV read verbatim was **2.42x SLOWER** (max_seq 1408 vs ~478 live); fixed by `DS2_KV_BUCKET` (default 256). Gates: `DS2_LEGACY_DECODE` / `DS2_FAST_DECODE` / `DS2_KV_BUCKET` / `DS2_KV_F16` (precision, unquantified, off) / `DS2_FORCE_CPU`. **Blocking infra bug found:** a stale `GGML_METAL_EMBED_LIBRARY=OFF` cache entry makes Metal silently fall back to CPU while `CMakeCache.txt` still reads `GGML_METAL:BOOL=ON` — affects every Metal claim from a `build/` binary, unowned (LEARNINGS.md). **Also found, not fixed:** the lane has NO repetition guard though the gold contract specifies `no_repeat_ngram_size=20` — 2 of 5 CC0 pages spiral into the 1024 cap (`FinlandFinland...`) and that alone drives cc0 CER 1.06; on the 3 pages that terminate, `receipt_historical` **beats** the reference (0.1198 vs 0.3633). CER numbers are pre-`tokenize-simple-audit` and need a post-merge re-gate | **DONE (verify+merge)** |
