...
| 2026-08-01 | `chore/ai-act-policy` / `.codex/worktrees/chore-ai-act-policy` | **Picked:** EU AI Act / biometric-misuse audit remediation. Adds `POLICY.md` (intended purpose, prohibited uses, obligations that transfer to deployers), a README "Intended purpose & acceptable use" section, and a `--accept-biometric` / `CRISPEMBED_ACCEPT_BIOMETRIC` acknowledgement gate on face **recognition** models (detection ungated). Reframes `examples/face_search.py` (1:N gallery search) → `examples/face_verify.py` (1:1), and drops the hardcoded `cos > 0.4 → MATCH` verdict from the CLI face-pipeline. Fixes `hf-space/README.md`, which advertised face recognition the Space does not implement. Touches ONLY `POLICY.md`, `README.md`, `PLAN.md`, `hf-space/README.md`, `examples/face_verify.py`, `examples/cli/{main.cpp,model_mgr.cpp,model_mgr.h}`, `examples/server/server.cpp` — **no `src/`, no `ggml/`, no model/OCR code** | **IN PROGRESS** |
