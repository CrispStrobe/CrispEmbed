...
|-------|-------------------|------|--------|
| 2026-08-06 | `fix/cuda-rec-zero-results` / `.claude/worktrees/fix-cuda-rec-zero-results` | **Claimed (Fable task 1): CUDA-rec 0-results bug** — read the conv-ab v2 rec-debug capture, localize post-graph (logits readback layout vs CTC decode), fix, prove with decoded text on CUDA (Kaggle round-trip per LEARNING 35). Unblocks O11 (ppocr det CUDA residency flip) | IN PROGRESS |
