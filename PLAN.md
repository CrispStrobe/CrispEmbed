...
|-------|-------------------|------|--------|
| 2026-08-05 | CrispASR `.claude/worktrees/feat-g8-gpu-pref` (claim pushed to their main `088e9318`) | **Claimed (G8=F10, round-6 coordinator's own work):** sync the T18 `--gpu-backend cpu` short-circuit logic into CrispASR's `gpu_backend_pref.h` + the PLAN #88 pipeline-cache write-path decision (patch is in the shared ggml fork) | **IN PROGRESS** |
| 2026-08-05 | *(round-6 delegated; coordinator verifies before merge)* | **Claimed: mxbai erf-vs-tanh GELU A/B** (G7c rider) — DeBERTa ContextPooler tanh-approx vs HF erf-exact; judged by decoded rerank scores vs ONNX Runtime reference (local torch disqualified); shipped-artifact default must stay byte-identical unless the A/B wins | **IN PROGRESS** |
| 2026-08-05 | *(round-6 delegated; coordinator verifies before merge)* | **Claimed: `CRISPEMBED_*_BENCH` presence-gate audit** — value-parse the 8+ engines' `getenv(...BENCH) != nullptr` gates (same `=0`-inverts class as the DS_ audit); compile+smoke per engine, three spellings proven on one engine | **IN PROGRESS** |
| 2026-08-05 | *(round-6 delegated; coordinator verifies + decides pins)* | **Claimed: reranker imatrix re-collection** (F7b leftover) — t19 Kaggle pipeline on the `-g7c` ms-marco artifacts + other pre-F7 reranker imatrix files; conversion mode must match imatrix names (read the quantizer's "N with imatrix" line); pin decisions stay with the coordinator | **IN PROGRESS** |
