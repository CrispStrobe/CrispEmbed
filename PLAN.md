...
|-------|-------------------|------|--------|
| 2026-08-07 | `perf/o3-pix2struct-sched` | **Claimed (round N+1 task 6, Opus-tier): O3 pix2struct enc_sched → {gpu, cpu}.** Mirror the got_ocr/R4 pattern (backend + cpu fallback + guarded `set_n_threads`) behind a per-engine gate; A/B on Metal with decoded output + interleaved timing pairs; record win/loss either way; fix stale "prefer GPU backend" comments | **IN PROGRESS** |
