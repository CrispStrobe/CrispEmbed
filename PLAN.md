...
| 2026-07-31 | `feat/ppocr-next-20260731` | **Picked:** O11.2 PP-LCNet line/page orientation graph port: backend-scheduled depthwise/pointwise/SE blocks with CPU fallback and orientation parity gates; graph now uses the converter's canonical weight layout, correct `[1280,2]` linear view, and safely schedules Metal plus CPU for unsupported depthwise ops. Metal parity test passes on the German line fixture with logit deltas 0.012/0.025; full corpus gate remains pending and production still requires `PPLCNET_ORIENTATION_GRAPH_ACCEPT=1` | **IN PROGRESS** |
