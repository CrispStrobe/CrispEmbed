...
|-------|-------------------|------|--------|
| 2026-08-06 | `fix/sched-replay-crash` / `.claude/worktrees/fix-sched-replay` | **Claimed (Fable task 5): ggml-fork sched alloc-once/compute-many replay crash** — repro `UOCR_PD=1 UOCR_PD_REPLAY=1` on Metal (dies gen=2 in op encode, 4/4 reproducible per the O6 row); localize in the fork's sched/Metal path with the re-apply-markers discipline; deepseek's working PD (`DS_*` twin) is the reference to diff against | IN PROGRESS |
