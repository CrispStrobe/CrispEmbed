...
| 2026-07-13 | `feat/ds-ocr2-stacked-experts` | DeepSeek-OCR-2 #4: converter-emitted stacked MoE experts (drop ~1.3 GB dup) | **Code landed on branch (`5bd3a97`), Kaggle reconvert RUNNING.** Converter emits `l.blk.{i}.ffn_{gate,up,down}_exps.weight` `[in,out,n_exp]` (byte-identical to runtime `stack_moe_experts`, verified locally). Loader loads them directly + builds per-expert views for the `DS_MOE_CPU` fallback + backward-compat for old per-expert GGUFs; quantizer already handles 3D experts (q4_k per-row, 1280%256==0). Kaggle kernel `chr1s4/crispembed-deepseek-ocr2-stacked-convert` reconverts `deepseek-ai/DeepSeek-OCR-2`, byte-validates stacked slices vs source, quantizes q4_k, uploads f16+q4_k to `cstr/deepseek-ocr2-crispembed-GGUF` as NEW `-stacked` files (non-clobbering — rev-pinned regression GGUF untouched). Next: local Metal decode + RSS check, then promote + land. |
