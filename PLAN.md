...
|-------|-------------------|------|--------|
| 2026-08-07 | `perf/pix2struct-cuda-decode` / `.claude/worktrees/perf-pix2struct-cuda-decode` (tip `64e0c9a3`) | **Round N+4 queue #1 checkpoint:** `CRISPEMBED_PIX2STRUCT_GGML_DECODE` shipped opt-in on the branch — single-backend ggml decode-step graph, device-resident self/cross KV (got_ocr pattern), gallocr reserved once, T5 rel-bias as per-step input. Decoded output BYTE-IDENTICAL to scalar locally: fox + scan_strip × textcaps f16 + q8_0 × CPU + Metal (MTL0 proven). No local timing verdict (box contended, recorded honestly). CUDA A/B kernel `chr1s4/crispembed-pix2struct-decode-ab` v1 RUNNING (4 arms, one lever each, proof-of-work rows) | **IN PROGRESS — awaiting CUDA A/B** |
| 2026-08-07 | *(kernel `chr1s4/crispembed-dbnet-rt` v1, script on `perf/pix2struct-cuda-decode`; runtime off `main`)* | **Round N+4 queue #2 checkpoint:** dbnet auto-CUDA remaining gate — CUDA decoded-text roundtrip through the full dbnet+TrOCR pipeline RUNNING: det CPU vs CUDA, 2 interleaved reps/arm (per-arm determinism), byte compare + per-line diff + arm-vs-arm CER + fox ground-truth CER. If text-equivalent, the `gpu_backend_pref.h` per-backend-kind default (CUDA→GPU, Metal/CPU unchanged) is mechanical per the ppocr O11 pattern | **IN PROGRESS — awaiting CUDA roundtrip** |
