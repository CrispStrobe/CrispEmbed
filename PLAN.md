...
| 2026-08-01 | `feat/ppocr-next-20260731` | **Picked:** O11.7 PP-OCRv6 recognizer weight-cache optimization: cache immutable convolution dequantization across line crops; retain the static-shape GGML scheduler allocation across crops; standalone artifacts now report incompatible tensor maps instead of failing silently. Corrected graph input staging from reference `[C,H,W]` to contiguous ggml `[W,H,C]`, corrected asymmetric graph convolution argument order (`x/y` versus reference `height/width`), and fixed the head shape from the accidental `6x5` map to the expected `10x1`/10-token map. Added debug-only channel-plane graph-tap comparisons and immediate staged-input verification; the input handoff is exact (`cosine 1.0`), but the earliest graph divergence is already at stem1 (`cosine 0.073657`), while stage4 is `0.002086` and decoded output remains `1：3` versus CPU `HI` with logit cosine about `0.35`. A direct-depthwise kernel and alternate ordinary-convolution weight-layout experiment did not improve parity and remain diagnostic-only. Acceptance remains disabled. Live German tiny pipeline remains `30` regions/`139` chars at confidence `0.93`; graph diagnostic computes logits then intentionally falls back. Tier audit: tiny graph builds through logits but decoded parity is not met; small/medium remain `large_stem=1` CPU paths. Graph is diagnostic-only and CPU fallback preserves `141/141` pipeline quality | **IN PROGRESS** |
