...
| 2026-07-13 | `feat/flova-omr` | Flova/omr_transformer — handwritten/whiteboard OMR (donut-swin + mBART VED → LilyPond, Apache-2.0) | 🔄 **Engine + wiring + quant done (branch)** — `src/flova_ocr.cpp`, `tests/test_flova_diff.cpp`, CMake, CLI dispatcher (`--ocr` auto-detect `flova_ocr`). f32 diff: all stages cos 1.000000, 40/40 argmax, byte-exact decode. q8_0 (573→162 MB): byte-exact decode on all 3 samples (patch/rpb auto-kept F32, no keep-guard needed); enc_output worst-token cos 0.987 = q8_0 floor, not a bug. **Pending: HF upload (`cstr/flova-omr-GGUF`), registry entry, regression fixture.** |
