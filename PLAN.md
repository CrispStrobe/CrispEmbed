...
| **layout_detect Phase-2 `cpu_linear` scalar matmul → contiguous SIMD AXPY** | `layout_detect.cpp:1018` | **DONE (byte-identical); speedup UNMEASURED — box was saturated** | Measured first (`CRISPEMBED_LAYOUT_DETECT_BENCH`): Phase-2 decoder (~25–48s) dominates (Phase 1 ~11–13s, heads ~5ms), and the cost is `cpu_linear` (`:1018`) — a scalar, un-SIMD, **stride-N** triple-loop matmul (dims up to 256×256×8400, ~10×/layer×6) — NOT the deformable sampling loop (a corrected guess). Rewrote the inner compute as `y[o,:] = b[o] + Σ_i W[o,i]·x[i,:]` **contiguous AXPYs over N**: `x[i,:]` and `y[o,:]` are contiguous in the col-major `[dim,N]` layout, so it vectorizes, while the **per-output accumulation order (i ascending) is unchanged → byte-identical** (verified: `diff` of the 20-region output vs the pre-change build is empty). Strided-scalar→contiguous-SIMD-AXPY for N up to 8400 is a guaranteed speedup, but **the actual number is unverified**: this session's machine hit loadavg 90–137 with a competing `crispasr`+flutter and 25% free RAM — per handoff rules, timing A/B under that load "fabricates wins", and running the heavy f32 model risked OOM. **TODO: A/B on a quiet box** (build a pre-AXPY baseline binary, run both back-to-back ≥3×). |
