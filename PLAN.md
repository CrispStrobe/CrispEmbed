...
|-------|-------------------|------|--------|
| 2026-08-03 | `feat/ocr-followups` / `.claude/worktrees/feat-ocr-followups` | **Picked:** close my own two loose ends from the h2ovl work, then sweep registry health. (1) **Reconcile the unexplained q8_0 cosine** — the parity kernel reported `cos_min 0.905481` where the same comparison locally gives `0.988750`; hypothesis is backend (Kaggle CPU vs Metal), testable with `INTERNVL2_OCR_FORCE_CPU=1` on one box. An unexplained factor-of-10 gap in an acceptance metric is not something to leave in the record. (2) **Registry health** — pins, licences and URL reachability across all entries after today's churn (h2ovl-2b repointed to q8_0, q4_k withdrawn, adair f16). **Deliberately NOT doing:** re-converting the published h2ovl f16/q8_0 to carry `internvl2.chat_template` explicitly. The vocab inference is verified on both h2ovl checkpoints (2b and 800m) and new conversions already emit the key; re-uploading 6.7 GB and re-pinning to replace a working inference with an equivalent one is cost without a defect. Recorded so nobody re-derives the question. | **IN PROGRESS** |
