...
|-------|-------------------|------|--------|
| 2026-08-05 | `feat/ds2-dynamic-crop` / `.claude/worktrees/feat-ds2-dynamic-crop` | **Picked (round-3 G2, coordinator's OWN work):** DeepSeek-OCR2 dynamic-crop port per the F5 brief. CHECKPOINT: port LANDED on the branch (`82a45a59`, opt-in `DS2_CROP_MODE=1`) — blueprint line-by-line at pinned `aaa02f38`; SAM 48-grid (pos-embed bicubic-aa a=-0.75, rel-pos 127→95 align_corners=False — fixed a latent never-exercised interp mapping), query_768 selection, `[bos][crops][global][vsep]` layout. Crop-off fox gold EXACT on Metal; crop-on fox (4×1 grid, 833-tok prompt) also exact. 6-arm cc0/synth matrix running in `tests/results/g2/`. Note: `~/.cache/hf-regression` is GONE despite the round-3 env notes (per-expert gold artifact will re-download) | **IN PROGRESS** |
