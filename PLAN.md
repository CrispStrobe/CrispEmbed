...
|-------|-------------------|------|--------|
| 2026-08-07 | `perf/o7-pplcnet-mk` | **Claimed (O7 increment 2): pplcnet_orientation scalar conv path → R6 im2col/mk defaults** via `conv2d_prefs_scope` with engine `n_threads`; nt1 A/B on the textline-ori f16 model, label identity + process CPU time; record win/loss either way | **IN PROGRESS** |
