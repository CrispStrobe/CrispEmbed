...
|-------|-------------------|------|--------|
| 2026-08-06 | `perf/r6-gemm-microkernel` / `.claude/worktrees/perf-r6-gemm` | **Claimed (Fable task 3): R6 register-blocked GEMM micro-kernel** in `core/cpu_ops.h` — justified by the x86 interchange win (nt=1 +13%, small-L2 story). Changes accumulation order ⇒ forfeits byte-equality ⇒ per-engine decoded-output A/Bs, bitwise tile path stays the reference arm; M1 pairs local, x86 verdict via a conv-ab-style kernel | IN PROGRESS |
