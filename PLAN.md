...
| 2026-08-01 | `feat/ppocr-next-20260731` | **Picked:** O11.7 PP-OCRv6 recognizer weight-cache optimization: cache immutable convolution dequantization across line crops; retain the static-shape GGML scheduler allocation across crops; standalone artifacts now report incompatible tensor maps instead of failing silently. Corrected the graph head pooling dimension order (`[W,H]`, yielding the expected 30 tokens) and added graph/CPU taps; ordinary graph convolutions now materialize the converter's Paddle-to-ggml layout explicitly. A CPU direct-depthwise stride experiment did not close the head mismatch and remains diagnostic-only. The remaining depthwise head convolution still collapses to a repeated bias-like activation: `HI` still decodes blank (`graph-vs-CPU logit cosine=0.2629`, max error `23.77`), so activation-reference parity must not be confused with decoded-output parity. Live German tiny pipeline remains `30` regions/`139` chars at confidence `0.93`; graph diagnostic is `3.46 s` versus `2.44 s` CPU because it computes logits then intentionally falls back. Tier audit: tiny graph builds through logits (`6906x30`, 19--55 ms/crop) but decoded parity is not met; small/medium remain `large_stem=1` CPU paths. Graph is diagnostic-only and CPU fallback preserves `141/141` pipeline quality | **IN PROGRESS** |
