...
|-------|-------------------|------|--------|
| 2026-08-07 | `perf/o7-mk-adoption` | **Claimed (round N+1 task 5, Opus-tier): O7/MK per-engine adoption.** Per-engine conv2d prefs hook (env keeps precedence both directions), then flip default-CPU conv paths one A/B at a time — SR family (text_sr) + ppocrv6-det scalar fallback first; engine `n_threads` must reach the call; byte-identity + process-CPU-time pairs per engine | **IN PROGRESS** |
