...
| 2026-08-02 | `feat/ocr-followups` / `.claude/worktrees/feat-ocr-followups` | **Picked:** the PARITY half of the h2ovl-2b port — `tools/kaggle/h2ovl-parity/`. Coordination: `chr1str/h2ovl-2b-convert` is another session's and was RUNNING when I looked, so I did **not** re-push it (kaggle_usage #25 — a push stacks a second session, it does not cancel); I pulled its live source and confirmed it is the post-fix version. That kernel does convert → q4_k → OCR smoke → upload, which is **not** a cleared model by `crispasr-crispembed-dev.md`: no `-ref.gguf`, no per-stage diff, one quant. This kernel adds the missing regime: bake `-ref.gguf` via `tools/dump_internvl2_reference.py` (numpy-only forward, no 8 GB torch load) → upload it to the model repo → re-download the **published** f16 (validate what users get, not a local rebuild) → `crispembed-quantize` q8_0 → `test-internvl2-diff` on f16/q8_0/q4_k → decoded-output OCR roundtrip on each → upload q8_0 **only** if it passes both gates. Has a degenerate-output detector because this model's recorded failure is exactly that (f16 repeated one token, q4_k emitted `.`), which a cosine gate alone would not catch. Verified before push: all 9 harness APIs exist with matching signatures, `py_compile` clean, metadata slug matches title, both `chr1str` datasets confirmed to exist via `dataset_list(mine=True)` — the `kaggle_usage.md` claim that only chr1s4 owns `crispasr-ccache` is stale and I corrected the doc. | **IN PROGRESS — kernel written; waiting on convert to publish the f16** |
