...
| 2026-07-13 | `feat/transcoda-omr` | Transcoda-59M zero-shot OMR (full-page score → Humdrum `**kern`; ConvNeXt-V2-tiny enc + 8-layer RoPE cross-attn decoder). **Clean-room** (weights CC-BY-4.0, code AGPL — engine written from paper + config + oracle only) | **VALIDATED — byte-exact decode.** Engine `src/transcoda_ocr.{h,cpp}` + converter + oracle + wiring (CMake/dispatcher/CLI/registry) + quantizer conv keep-guard. f32 `test-transcoda-diff` **all stages cos=1.000000** (CPU & Metal) + **argmax 191/191** + native preproc bit-exact. **Greedy decode byte-identical to HF oracle** (460 chars / 203 tokens). q8_0 (65 MB, 3.4×): argmax 191/191, **decode byte-identical to oracle** (enc cos 0.988 = q8_0 conv floor, output exact). Fixed: KV view-stale, rep-penalty per-unique-token, `/`-separator. Next: push `main`, upload `cstr/transcoda-omr-GGUF` (CC-BY card), regression fixture. |
