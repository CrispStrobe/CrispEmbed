...
| KV cache | DONE | Prefill + single-token decode, matches the full recompute (and HF) token-for-token. Two latent bugs fixed: (1) k_out/v_out were pruned from the graph (side outputs, not ancestors of the logits) → extraction returned null → silent no-cache; (2) the V side output was a reshape *view* the no-alloc scheduler reused → garbage cached V. Fix: `ggml_build_forward_expand` each side output, and `ggml_cont` K_new/V_new before marking them outputs. `CRISPEMBED_NO_KV_CACHE=1` forces the exact full-recompute path for A/B. |
