...
|-------|-------------------|------|--------|
| 2026-07-17 | `feat/embeddinggemma-community-gguf` | **Community/official `gemma-embedding` EmbeddingGemma GGUFs crash on load** (encoder QKV reshape overrun — it's GQA not MHA). Step 1 = arch-gated routing to `decoder_embed.cpp` (3 edits). | **OPEN — step 1 done LOCALLY, NOT shipped.** Routing verified: loads 24L/768d, 3 heads/1 kv, mean pool, bidirectional, no crash. But garbage-guard margin only **0.038** (cos rel 0.885 / unrel 0.847) — GGUF has no Dense/Matryoshka modules. **Never ship step 1 alone** (silent-weak = modern-bert anti-pattern). Next: disentangle missing-Dense vs gemma-norm `1+w` by validating the pre-Dense backbone vs HF `google/embeddinggemma-300m`. Recipe: `handover-prompts/embeddinggemma-community-gguf.md`. |
