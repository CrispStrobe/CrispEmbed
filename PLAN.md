...
|-------|-------------------|------|--------|
| 2026-08-07 | `fix/glm-thin-strip` | **Claimed (round N+1 task 3): GLM thin-strip hallucination.** glm_ocr invents continuation sentences on wide-short inputs (`scan_strip` repro, recorded in the round-2 GLM rows). Reproduce on current main, localize (stop criterion / attention sink / prompt), fix gated per protocol, A/B on the strip + clean fixtures | **IN PROGRESS** |
