...
|-------|-------------------|------|--------|
| 2026-08-05 | `fix/rerank-bert-pooler` / `.claude/worktrees/fix-rerank-bert-pooler` | **Claimed (round 5): G7c expanded — shipped ms-marco rerankers are missing the BertPooler stage.** Finding: `cstr/ms-marco-MiniLM-L-6-v2-GGUF` (f16 + all quants) has only 1-layer `classifier.{weight,bias}` while the HF checkpoint carries `bert.pooler.dense.*`; HF scores `classifier(tanh(pooler(CLS)))`, native scores `dot(CLS,w)+b`. The LEARNINGS 2026-07-03 "ms-marco uses tanh, verified" claim does NOT hold for the shipped artifact. Plan: measure HF-vs-native score divergence, then converter fix (map BERT pooler.dense→classifier.dense + classifier→classifier.out_proj = the verified 2-layer tanh path; suppress the separate pooler emit for that case), re-convert, verify vs HF, coordinator decides re-upload/re-pin | **IN PROGRESS** |
