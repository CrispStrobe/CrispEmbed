...
|-------|-------------------|------|--------|
| 2026-08-05 | `fix/ds-env-value-parse` / `.claude/worktrees/fix-ds-env-value-parse` | **Claimed (round 5): DS_ value-parse audit** (the G2b follow-up row below) — convert the presence-based `DS_*` gates in `src/deepseek_ocr2.cpp` (`DS_MMAP`, `DS_MOE_CPU`, `DS_DBG`, `DS_SAM_CONV_CPU`, `DS_QWEN2_ENC_FLASH`, `DS_QWEN2_SCALAR`, `DS_LLM_FLASH`, `DS_NO_KV`, `DS_LMHEAD_CPU`) to value-parse per the 73beea9f/8c210291 pattern; per-gate three-spelling verification (absent/`=0`/`=1`) on real runs, one variable per A/B; debug-only gates compile+smoke. Note: v0.17.5 release is owned by the parallel session that drafted `RELEASE_NOTES_v0.17.5.md` (`1000a746`) — not claimed here | **IN PROGRESS** |
