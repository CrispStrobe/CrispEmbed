...
| 2026-08-04 | `feat/t14-deepseek2-decode-graph` (agent) | **T14** — deepseek_ocr2 persistent decode-step graph + F16 KV, copying qwen2vl `build_decode_step_graph` (`src/qwen2vl_ocr.cpp:2313`). FIRST add the missing `[deepseek-ocr2-stage-bench]` net-of-load line (T12 gap). Gate: CER vs `tests/regression/gold/deepseek-ocr2/` (threshold, never byte-diff) + interleaved same-window A/B. Holds the box's single heavy-process slot; timing runs quiet-machine only | IN PROGRESS |
...
| 2026-08-04 | `feat/t15-smoldocling-doctags` (this session) | **T15** — SmolDocling DocTags dedup: live payload CER 0.86 from duplicated DocTags despite 0.9999 tensor parity — diff the OUTPUT CONTRACT vs the reference implementation's own DocTags on gold pages; only then un-hardcode the CPU backend | IN PROGRESS |
