...
|-------|-------------------|------|--------|
| 2026-08-06 | `infra/ccache-verify` | **Claimed (round N+1 task 7): FIRST KERNEL — verify the crispembed-ccache seed fires.** Attach `chr1s4/crispembed-ccache` to the `crispembed-ccache-seed` kernel, re-push under chr1s4, require `ccache: warmed from …` in the log (a `cold build` line = seed regressed, see dev-guide addenda); then `datasets version` the freshly exported `ccache.tar` to keep the seed current | **IN PROGRESS** |
| 2026-08-06 | `fix/pageseg-round3` | **Claimed (round N+1 task 1): pageseg round 3 — baseline-clustered line assignment.** First fix the index-bogus ROW_DEBUG `out=` label, then replace the greedy y-adjacency fragment chain (6 px median fragments, `max_row_gap=4`) with baseline-clustered assignment. Target Fraktur CER 0.271 → ≤0.235 at ~1.2 s; re-measure the round-2 row map on current main before coding | **IN PROGRESS** |
| 2026-08-06 | *(docs only, main)* | **Claimed (round N+1 task 9): hygiene** — archive this round's DONE rows to HISTORY; add rows 22-24 to `tools/upstream-prs/README.md` status table | **QUEUED** |
