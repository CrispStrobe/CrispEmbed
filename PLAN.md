...
|-------|-------------------|------|--------|
| 2026-08-06 | `perf/r6-gemm-microkernel` (branch pushed; kernel `chr1s4/crispembed-mk-ab` v1 RUNNING) | **Fable task 3: R6 micro-kernel BUILT + M1-PROVEN, x86 gate in flight.** Outer-product consume on packed [k][r] weight / [k][c] column blocks (NEON 4x4 vfmaq_laneq, AVX2 8x4 broadcast-fmadd, scalar fallback; dot-consume remainders), opt-in `CRISPEMBED_CONV2D_MK=1` (implies GEMM path); bitwise tile path untouched as reference arm, mk arm tolerance-gated ≤1e-4 (**196/196 unit checks**, 9 shapes × nt=1,4). **M1 A/B (medium scalar det page, 3 pairs): wall load-garbage, CPU time decisive — user 25.8-27.0 → 17.7-18.0 s (−34% of process CPU), decoded text byte-identical** — a win even where the plain interchange lost. Kernel runs the AVX2 unit gate (first-ever x86 compile of the mk path) + 5 CPU-time arms (legacy/gemm×2/mk×2); merge AFTER it lands (an AVX2 compile break must not reach main) | IN PROGRESS |
