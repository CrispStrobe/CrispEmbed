...
|-------|-------------------|------|--------|
| 2026-08-06 | `fix/vlm-ram-guard` / `.claude/worktrees/fix-vlm-ram` | **Claimed (SubtitleEdit-13238 follow-ups): VLM RAM preflight + CPU-decode perf measure-first** — (a) deepseek froze a 4 GB host 3/3: add a model-size-vs-available-RAM preflight (refuse/warn, env-overridable) to the big-VLM engines; (b) profile glm/deepseek CPU-only decode on M1 to name the perf levers before touching anything; (c) check whether crispstrobe-ops `00285218` (cpu conv_2d im2col vec_dot_type) is needed on sync/upstream-v0.17 | IN PROGRESS |
