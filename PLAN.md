...
|-------|-------------------|------|--------|
| 2026-08-07 | `perf/pageseg-round5` / `.claude/worktrees/perf-pageseg-round5` | **Claimed (round N+3 task 2): pageseg round 5 — the decoder levers.** Measure `CRISPEMBED_TESSERACT_RECODE_BEAM_WIDTH` / `_DAWG_SCORE` / `_DAWG_PREFIX_SCORE` post-int8-cache on the Fraktur fixture (CER + time, interleaved pairs), then a recognition-confidence signal for the ~60 ornament junk chars. 24-fixture arms gate before any default change; segmentation geometry NOT re-opened | **IN PROGRESS** |
