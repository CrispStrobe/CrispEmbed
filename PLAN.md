...
|-------|-------------------|------|--------|
| 2026-08-06 | `perf/layout-o2b-valueproj` / `.claude/worktrees/perf-layout-o2b` | **Claimed (Fable task 6): layout O2b value-proj on GPU** — re-profile Phase 2 on current main FIRST (flat-im2col moved the layout numbers: call 1.56→0.95 s), then the value-projection GPU move (was est. 101→~30 ms at `-t 4`) if the profile still names it; hand-written graph math, byte/region-identity + interleaved pairs per protocol | IN PROGRESS |
