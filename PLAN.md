...
| 2026-07-13 | `feat/flova-omr` | Flova/omr_transformer — handwritten/whiteboard OMR (donut-swin + mBART VED → LilyPond, Apache-2.0) | 🔄 **Engine validated (branch, not yet on `main`)** — `src/flova_ocr.cpp` + `tests/test_flova_diff.cpp` + CMake wired. Diff harness vs f32 ref: all stages cos 1.000000 (enc_stage0..3, enc_output, dec_block0..3, logits), 40/40 argmax agree, greedy decode byte-exact to model card (`c'2 a''8 c''8 r4 c'1 e'8 c'8 c'8 a''8 f'4 a'8 c'8`); native Donut-preproc file path also byte-exact on sample1. **Pending: CLI dispatcher wiring, q8_0 quant, HF upload (`cstr/flova-omr-GGUF`), registry + regression fixture.** |
