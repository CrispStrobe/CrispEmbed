...
8. **GLM vision parity** [REFRAMED 2026-08-07, `fix/glm-refdump-vision`]: the
   premise was wrong — the merger tap is not the first divergence and is not
   known-stale; and `cos_min` was being measured over the wrong axis for
   EVERY engine (fixed). The real divergence is a **layer-13 cliff** in the
   vision stack, with quantization, f16 conversion and per-layer structure
   all ruled out by measurement. **Blocked on arbitration**: deciding port
   vs dumper needs the real HF forward, and the GLM-OCR weights are not in
   the local cache (~2.6 GB; the designated volume is at 100%). Unverified
   lead at the code site: the HF 2x2 merge groups four CONSECUTIVE sequence
   positions, relying on the image processor's block pre-ordering that the
   synthetic diff path bypasses.
