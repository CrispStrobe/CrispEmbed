...
| 2026-08-03 | `feat/ocr-followups` / `.claude/worktrees/feat-ocr-followups` | **h2ovl-2b: my half done; invocation half resolved by the parallel session (`fcebf561`) — no overlap, and their fix builds on mine.** Sequence for the record: I found the graph was NOT at fault (baked the `-ref.gguf`, `test-internvl2-diff` clean at f16) and that `build_prompt()` emitted InternVL2 ChatML for a `template: h2ogpt2` checkpoint whose vocab has no `<|im_start|>`/`<|im_end|>` — landed as `5f617351` (template dispatch + multi-token stop set). That got the decoder from a repeated token to fluent prose but still describing rather than transcribing. The other session then found the remaining two: a **spurious BOS** (`add_bos_token: false` — **I verified this directly in the checkpoint's tokenizer_config**) which my h2ogpt2 branch was emitting, plus a too-terse instruction; neither works alone. **Their result: 2b transcribes 1806 chars verbatim.** **Still mine and still open — the quant recipe.** The two artifacts differ in exactly 168 tensors (the LLM `l.*` attn+FFN): at Q8_0 `llm_layer_0` is `0.998033`, at Q4_K it is `0.594995` and **anti-correlated** (`-0.268615`) by layer 2 — not drift, so **Q4_K is unusable for this arch** (2560d, 32H/8KV, head_dim 80). Separately the vision tower `v.*` (98 tensors) is Q8_0 in **both** files, which is why `vis_proj_output` is bit-identical `0.998630` in both while f16 clears `0.999972` — vision+projector want F16. `tools/quantize.cpp` has **no internvl2 rule** today; I am building/testing one rather than asserting it. **Do not publish a q4_k for this model.** **Harness fix (`e5db01f1`):** `test-internvl2-diff` printed `cos=1.000000 max_abs=0.000000 FAIL` for every layer absent from the reference — 20 fabricated rows that inflated the stage count (my kernel reported "27 stages" for a 7-stage reference). Guarded with `ref.has()`; any automated gate reading that output was being diluted by 20 free passes. **Artifacts:** `cstr/crispembed-regression-fixtures` → `internvl2/h2ovl-mississippi-2b/ref.gguf`; `cstr/h2ovl-mississippi-2b-crispembed-GGUF` PRIVATE + carded UNVALIDATED (f16/q8_0/q4_k). | **IN PROGRESS — verifying their fix end-to-end, then the quant recipe** |
