...
//
// ⚠ CROSS-REPO CONTRACT (issue #50). This header is the CrispEmbed half of a
// mirrored pair; the other half is CrispASR's src/core/ggml_cpu_backend.h. The
// shared libraries that live in CrispASR — crisp_audio, crisp_punc, crisp_lid,
// crisp_truecase — are pulled into THIS build with add_subdirectory() and then
// compile against the CONSUMER's src/core (crispembed-core's PUBLIC include
// dirs). Four of their translation units `#include "core/ggml_cpu_backend.h"`,
// so the file has to exist on both sides or a from-source CrispEmbed build next
// to a current CrispASR checkout dies with
//     fatal error: core/ggml_cpu_backend.h: No such file or directory
// which is exactly what issue #50 reported (against an older CrispASR, where
// the same drift surfaced as a std::map/std::unordered_map mismatch in
// audio_tower.cpp — see the tensor_map note in core/gguf_loader.h for that
// half). CrispEmbed does not build with GGML_BACKEND_DL, so only the direct-call
// branch below is ever compiled here and CrispEmbed's own ~150
// ggml_backend_cpu_init() sites are untouched; the file exists to satisfy the
// shared libraries. Keep it in sync with CrispASR's copy when that one changes
// — the two are byte-identical apart from this note and the pointer spacing
// (`T * p` here, `T* p` there), which each repo's own .clang-format decides.
// The CI guard is the `sibling-crispasr` job in .github/workflows/build.yml,
// which builds the four shared libraries against this tree's src/core.
//
