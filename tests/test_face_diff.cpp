...
int main(int argc, char ** argv) {
    if (argc < 3) { fprintf(stderr, "Usage: %s scrfd.gguf ref.gguf [image.png]\n", argv[0]); return 1; }
    ...
    if (!image) { fprintf(stderr, "face image not found\n"); return 1; }
    ...
    if (ok) {
        int best = 0; float bc = dets[0].confidence;
        for (int i = 1; i < n; i++) if (dets[i].confidence > bc) { bc = dets[i].confidence; best = i; }
        const auto & d = dets[best];
        float cpp[15] = { d.x, d.y, d.w, d.h, d.confidence,
            d.landmarks[0], d.landmarks[1], d.landmarks[2], d.landmarks[3], d.landmarks[4],
            d.landmarks[5], d.landmarks[6], d.landmarks[7], d.landmarks[8], d.landmarks[9] };
        for (int j = 0; j < 15; j++) {
            float e = std::fabs(cpp[j] - ref_det[j]);
            if (j == 4) conf_err = e; else if (e > max_px) max_px = e;
        }
