...
int main(int argc, char ** argv) {
    ...
    std::string hat_model_path;       // HAT super-resolution model (--hat-sr-model)
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(argv[i], "--hat-sr-model") == 0 && i + 1 < argc) hat_model_path = argv[++i];
    ...
    if (model_path.empty() && det_model_path.empty() && vit_model_path.empty() && math_ocr_model_path.empty() && layout_model_path.empty() && ner_model_path.empty() && sr_model_path.empty() && pan_model_path.empty() && hat_model_path.empty() && tbsrn_model_path.empty() && restormer_model_path.empty()) {
        ...
        fprintf(stderr, "\nHAT super-resolution (whole-image upscaling, CVPR 2023 SOTA):\n");
        fprintf(stderr, "  --hat-sr-model MODEL HAT SR GGUF (Hybrid Attention Transformer, 21M, 2x or 4x); enables POST /hat/sr\n");
    ...
    svr.Post("/hat/sr", [&](const httplib::Request & req, httplib::Response & res) {
        if (!hat_sr_ctx) {
            res.status = 503;
            res.set_content("{\"error\": \"no HAT SR model loaded (use --hat-sr-model)\"}", "application/json");
            return;
        }
