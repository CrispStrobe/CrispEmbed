...
int main(int argc, char ** argv) {
    ...
    if (!ocr_pipeline_path.empty()) {
        ...
        if (!output_format.empty() && output_format != "text") {
            ...
            float lang_conf = 0;
            const char * det_lang = crispembed_ocr_pipeline_detected_lang(pctx, &lang_conf);
            printf("{\"n_regions\":%d,\"mean_confidence\":%.3f", n_res, mean_conf);
            if (det_lang && *det_lang)
                printf(",\"detected_lang\":\"%s\",\"lang_confidence\":%.3f", det_lang, lang_conf);
            printf(",\"full_text\":\"%s\"}\n",
            ...
            const char * det_lang = crispembed_ocr_pipeline_detected_lang(pctx, &lang_conf);
            if (det_lang && *det_lang)
                printf("regions=%d  mean_conf=%.2f  lang=%s (%.2f)\n%s\n",
                       n_res, mean_conf, det_lang, lang_conf, full_text ? full_text : "");
            else
