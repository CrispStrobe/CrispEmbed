...
`LFM2_VL_CPU_PROJECTOR=1` (or `LFM2_VL_PROJ_GGML=0`) restores the scalar path.
...
### The same defect measured on M1 Metal (2026-08-24)

Independently, on an 8-core M1 with the Metal backend, `commons_example_receipt.png`
(500x650, single tile, Q4_K): projector **6581 ms → 8 ms**, 31% of a 63.7 s
pipeline. Two very different machines, the same conclusion — the scalar
projector was never a hardware-specific problem.

Three further M1-only findings from that session, none of which can appear on
CUDA or CPU (see `docs/lfm2_vl/PLAN.md` §7–§8):

| change | effect |
|---|---|
| stop stamping `GGML_PREC_F32` on the LLM attention — this fork's Metal backend REFUSES that op, so `sched` ran all 8 attention layers on the CPU, copying the whole KV cache both ways every token | decode 159 → 62 ms/token |
| read the KV cache as a strided view instead of `ggml_cont`-ing it per layer per token (238 MB/token at n_kv 1816) | a further 1.42x |
| `LFM2_VL_FLASH_ATTN` in the VISION encoder had a spurious trailing permute after `flash_attn_ext` (it already permutes internally) — cos_global 0.563 and hallucinated output; fixed and now default on | vision encoder 2237 → 1747 ms |

After those, on M1 the decode step profiles as build 0.5 ms, `sched_alloc` 3.1 ms,
inputs 0.02 ms, **compute 100 ms**, readback 0.6 ms — i.e. graph reuse and the
ShortConv state round-trip are worth at most ~3% and the cost is GPU compute
with no CPU split left in the graph.
